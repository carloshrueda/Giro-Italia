...
def menuEliminar(equipos, ciclistas, etapas, resuletapas):
    pass
                            ...
                            if ti < 1:
                                ...
                                if filEtaCi == -1:
                                    ...
                                    if tiempoant == 0:
                                        ...
                                        modifica = inputContinuarSN("Existe un tiempo registrado. Desea modificarlo?")
                                        if modifica == "S":
                                            resuletapas[filEtaCi][3] = ti
                    ...
                    if filanuci == -1:
                        ...
                        if len(kilometros) == 0:
                            ...
                            if km < 1:
                                ...
                                if km > kmetapa:
                                    # si los km recorridos por el ciclista son mayores que los km de la etapa
                                    # esto es un error
                                    print("***> Error, el ciclista no puede recorrer más Km que la etapa")
                                    print("     Km de la etpa: ", km)
                                else:
                                    # si los km recorridos por el ciclista son iguales o menores a los km de la etapa
                                    filEtaCi = existeEtapaCiclista(etapa, numero, resuletapas)
                                    if filEtaCi == -1:
                                        # No existe datos en la etapa de dicho ciclista
                                        # entoncse se agrega estos datos nuevos
                                        resuletapas.append([etapa, numero, km, 0])
                                    else:
                                        # existen datos para el ciclista en la etapa
                                        # entoncs modificar
                                        kmant = resuletapas[filEtaCi][2]
                                        if kmant == 0:
                                            # si es cero, modifica sin problema
                                            resuletapas[filEtaCi][2] = km
                                        else:
                                            # ya se habia registrado un tiempo
                                            modifica = inputContinuarSN(
                                                "Existe un tiempo registrado. Desea modificarlo?")
                                            if modifica == "S":
                                                resuletapas[filEtaCi][2] = km
...
def ingresarKilometrosxEtapas(etapas):
    ...
    opcion = ""
    while opcion != "N":
        etapa = input("\nNombre de la Etapa a quien desea ingresar los kilómetros? ")
        etapa = etapa.strip()
        if len(etapa) > 0:
...
def ingresarEtapas(etapas):
    ...
    opcion = ""
    while opcion != "N":
        etapa = input("\nNombre de la Etapa a quien desea ingresar? ")
        etapa = etapa.strip()
        if len(etapa) > 0:
            etapa = etapa.upper()
            fila = existeEtapa(etapa, etapas)
                    ...
                    if filanuci == -1:
                        ...
                        if len(nombre) > 0:
                            ...
                            if filanoci == -1:
                                ...
                                ciclistas.append([equipos, numero, nombre])
                            else:
                                print(
...
def inicio():
    ...
    while (opcion != "5"):
        ...
        while opcion < "1" or opcion > "5":
            print("***> Error. Opción inválida del menu. Escoja (1-5)")
            print("***> Presiones cualqueir tecla para continuar ...")
            input()
            menu()
        ...
        if opcion == "1":
            ...
            # 2. EDITAR
            menuEditar(equipos, ciclistas, etapas, resuletapas)
        elif opcion == "3":
            # 3. ELIMINAR
            menuEliminar(equipos, ciclistas, etapas, resuletapas)
        elif opcion == "4":
            # 4. CONSULTAR
